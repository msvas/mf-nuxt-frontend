            ...
            <div class="col-6 p-0">
              <div class="row">
                ...
                <div class="col-12 text-right">
                  <vs-button @click="deleteFromCart()" class="p-0 mr-1 font-small-3" style="margin-top: 8px;" color="#636363" type="flat">Excluir</vs-button>
                  <vs-button @click="updateEditModal(true)" class="p-0 font-small-3" style="margin-top: 8px;" color="#636363" type="flat">Editar</vs-button>
                </div>
              </div>
            </div>
          </div>
...
import PublicStoreProductModal from "@/components/products/PublicStoreProductModal";

export default {
  name: "ReviewOrderProductCard",
  data() {
    return {
      openPhoto: false,
      adding: false,
  ...
  methods: {
    ...
    checkTotals(val) {
      this.$emit('quantity-input', val)
    },
    removeFromCart(val) {
      this.$emit('remove-item', val)
    },
    deleteFromCart() {
      this.$emit('delete-item', this.expedition)
    },
  },
  props: {
    product: {
